...
class MobileNet():
    ...
    def build_Encoder_Layers(self, X, alpha = 1, bn_decay = 0.999, bn_eps = 1e-3, name = 'mobile_net'):
        ...
        with tf.compat.v1.variable_scope(name):
            with tf.compat.v1.variable_scope('reduce_input_block'):
                first_block_filters = self.make_Divisible(32 * alpha, 8)
                Layers.append(tf.compat.v1.layers.conv2d(Layers[-1], first_block_filters, (3 , 3), strides = 2, use_bias = False, padding = 'SAME', activation = None, kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform") ,name = 'convd_3x3'))
                Layers.append(tf.compat.v1.layers.batch_normalization(Layers[-1], axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name = 'bn_1'))
    ...
    def inverted_Res_Block(self, X, expansion, stride, alpha, filters, block_id, skip_connection, rate = 1, bn_eps = 1e-3, bn_decay = 0.999, chan_dim = -1):
        ...
        x = X
        with tf.compat.v1.variable_scope('inverse_Residual_Block' + str(block_id)):
            if block_id:
                with tf.compat.v1.variable_scope('expand'):
                    print('expand')
                    x = tf.compat.v1.layers.conv2d(x, expansion * chan_size, (1,1), use_bias=False,padding='same', activation=None, kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), name='conv2d_1x1')
                    x = tf.compat.v1.layers.batch_normalization(x, axis=chan_dim, epsilon=bn_eps, momentum=bn_decay,name='bn')
                    x = tf.nn.relu(x, name='relu6')

            with tf.compat.v1.variable_scope('depthwise'):
                print('depthwise')
                depthwise_filter = tf.compat.v1.get_variable('filters', (3,3, x.shape[-1],1), tf.float32)
            ...
            with tf.compat.v1.variable_scope('pointwise'):
                print('pointwise')
                x = tf.compat.v1.layers.conv2d(x, pointwise_filters, (1,1), use_bias=False,padding='same', activation=None, kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), name='conv2d_1x1')
                x = tf.compat.v1.layers.batch_normalization(x, axis=chan_dim, epsilon=bn_eps, momentum=bn_decay,name='bn')
                x = tf.nn.relu(x, name='relu6')
...
class ResNetV1():
    ...
    def build_Encoder_Layers(self, X, name = "ResnetV1"):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            with tf.compat.v1.variable_scope('reduce_input_block'):
                ...
                Layers.append(tf.compat.v1.layers.conv2d(Layers[-1], 64, 7,strides=(2, 2), use_bias=False, padding="valid", activation=None, name="conv2d_7x7"))
                Layers.append(tf.compat.v1.layers.batch_normalization(Layers[-1], axis=chan_dim, epsilon=self.bn_eps, momentum=self.bn_decay, name='bn'))
                Layers.append(tf.nn.relu(Layers[-1], name='relu'))
                Layers.append(self.ZeroPadding(Layers[-1], 3, "ZeroPadding_2"))
                Layers.append(tf.compat.v1.layers.max_pooling2d(Layers[-1], (3, 3), strides=(2, 2), padding="valid", name = 'max_pooling2d'))
    ...
    def Residual_Block_1(self, X, filters = 64, stride = 2, chan_dim = -1, shorcut = False, bn_eps = 2e-5, bn_decay = 0.9, name = "block1"):
        ...
        """
        with tf.compat.v1.variable_scope(name):
            x = tf.compat.v1.layers.conv2d(X, filters, (3 , 3), strides=stride, use_bias = False, padding = "same",  activation = None, name = name + 'conv2d_3x3_1')
            x = tf.compat.v1.layers.batch_normalization(x, axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name = name + 'bn_1')
            x = tf.nn.relu(x, name= name + 'relu_1')

            x = tf.compat.v1.layers.conv2d(x, filters, (3, 3), strides= (1, 1), use_bias = False, padding="same", activation = None, name = name + 'conv2d_3x3_2')
            x = tf.compat.v1.layers.batch_normalization(x, axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name = name + 'bn_2')

            # if we are to reduce the spatial size, apply a CONV layer to the shortcut
            if shorcut:
                X = tf.compat.v1.layers.conv2d(X, filters, (1, 1), strides = stride, use_bias = False, activation = None, name = name + "reduce_conv2d_1x1")
                X = tf.compat.v1.layers.batch_normalization(X, axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name = name + 'bn_3')
    ...
    def Residual_Block_2(self, X, filters = 64, stride = 2, chan_dim = -1, shorcut = False, bn_eps = 2e-5, bn_decay = 0.9, name = "block1"):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            x = tf.compat.v1.layers.conv2d(X, filters, (1, 1), strides=stride, use_bias = False, padding="same", activation = None, name = name + 'conv2d_1x1_1')
            x = tf.compat.v1.layers.batch_normalization(x, axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name = name + 'bn_1')
            x = tf.nn.relu(x, name='relu_1')

            x = tf.compat.v1.layers.conv2d(x, filters, (3, 3), strides=(1 , 1), use_bias = False, padding="same", activation = None, name= name + 'conv2d_3x3_1')
            x = tf.compat.v1.layers.batch_normalization(x, axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name=name + 'bn_2')
            x = tf.nn.relu(x, name = 'relu_2')

            x = tf.compat.v1.layers.conv2d(x, 4 * filters, (1, 1), strides=(1 , 1), use_bias = False, padding="same", activation = None, name= name + 'conv2d_1x1_2')
            x = tf.compat.v1.layers.batch_normalization(x, axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name = name + 'bn_3')



            if shorcut:
                X = tf.compat.v1.layers.conv2d(X, 4 * filters, (1, 1), strides = stride, use_bias = False, activation = None, name=name + "shorcut_conv2d_1x1")
                X = tf.compat.v1.layers.batch_normalization(X, axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name = name + 'shorcut_bn_4')
...
class ResNetV2():
    ...
    def Residual_Block_1(self, X, filters = 64, stride = 2, chan_dim = -1, shorcut = False, bn_eps = 2e-5, bn_decay = 0.9, name = "block1"):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            x = tf.compat.v1.layers.batch_normalization(X, axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name = name + 'bn_1')
            x = tf.nn.relu(x, name = name + 'relu_1')
            x = tf.compat.v1.layers.conv2d(x, filters, (3, 3), strides=stride, use_bias = False, padding = "same",  activation = None, name = name + 'conv2d_1x1_1')
    ...
    def Residual_Block_2(self, X, filters = 64, stride = 2, chan_dim = -1, shorcut = False, bn_eps = 2e-5, bn_decay = 0.9, name = "block1"):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            x = tf.compat.v1.layers.batch_normalization(x, axis = chan_dim, epsilon = bn_eps, momentum = bn_decay, name = name + 'bn_3')
            x = tf.nn.relu(x, name = name + 'relu_2')
            x = tf.compat.v1.layers.conv2d(x, 4 * filters, (1, 1), strides = (1 , 1), use_bias = False, padding = "same", activation = None, name = name + 'conv2d_1x1_3')

            # if we are to reduce the spatial size, apply a CONV layer to the shortcut
            if shorcut:
                X = tf.compat.v1.layers.conv2d(X, 4 * filters, (1, 1), strides = stride, use_bias = False, activation = None, name = name + "reduce_conv2d_1x1")
...
class Xception():
    ...
    def build_Encoder_Layers(self, X, name = 'xception'):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            Layers.append(tf.compat.v1.layers.max_pooling2d(Layers[-1], 2, 2, name=name + '_maxpooling_1'))
    ...
    def general_conv2d(self, input_data, filters=64,  kernel_size=7, stride=1, conv_type = 'conv', stddev=0.02, activation_function="relu", padding="VALID", do_norm=True, relu_factor=0, name="conv2d"):
        with tf.compat.v1.variable_scope(name):
            if conv_type == 'conv':
                conv = tf.compat.v1.layers.conv2d(input_data, filters, kernel_size, stride, padding, activation=None, kernel_initializer=slim.initializers.xavier_initializer())
            if conv_type == 'dep_conv':
                conv = slim.separable_conv2d(input_data, filters, kernel_size, 1, stride, padding, activation_fn = None, weights_initializer=slim.initializers.xavier_initializer())
